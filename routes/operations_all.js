...
handoverRouter.get('/', requireAuth, (req, res) => {
  const date = req.query.date || new Date().toISOString().split('T')[0];
  let q = `SELECT h.*, u.display_name as author_name, u.service as author_service FROM handovers h LEFT JOIN users u ON h.created_by = u.id WHERE h.hotel_id = ? AND h.date = ?`;
  const params = [req.session.hotelId, date];
  if (req.query.service) { q += ' AND h.service = ?'; params.push(req.query.service); }
...
handoverRouter.post('/', requireAuth, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO handovers (hotel_id, date, shift, service, content, created_by) VALUES (?, ?, ?, ?, ?, ?)').run(req.session.hotelId, date || new Date().toISOString().split('T')[0], shift, service || req.session.service || 'general', content, req.session.userId);
